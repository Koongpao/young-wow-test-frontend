"use client";
import { getPostById, updatePost } from "@/app/lib/controller";
...
export default function Page({params} : {params: {id: string}}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const response = await updatePost(title, content, params.id);
  ...
  return (
    <div>
      <div className="flex flex-col bg-slate-300 items-center justify-start min-h-screen p-8 gap-y-6 sm:p-20 font-[family-name:var(--font-geist-sans)]">
        <h1 className="text-4xl font-bold text-slate-600">Editing Post</h1>
        <div>
